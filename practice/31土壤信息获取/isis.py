...
def get_need_data(ID5):
    #init
    TITLE, Date, Lat_Lon, land_use1, land_use2, land_use3, land_use4, land_use5, wrd2006, wrd1998, fao1974, fao1988, depth = "","","","","","","","","","","","",""
    ...
    try: 
        Date  =  tree.xpath('//td[contains(text(), "Date")]/following-sibling::td[1]/text()')[0]
    except:
        Date = ""
    try: 
        Lat_Lon  =  tree.xpath('//td[contains(text(), "Lat/Lon")]/following-sibling::td[1]/text()')[0]
    except:
        Lat_Lon = ""
...
def get_isis():
    ...
    #实例化一个线程池对象
    pool = Pool(20) # 4 个进程
    #注意，传递ids不是单个值，而是将列表传递给get_need_data进行处理。
    pool.map(get_need_data, ids) #<<<<<<<<<<<<<<<<<<<<<<<<
    pool.close()
    pool.join()
